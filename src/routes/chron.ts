	...
	for (let i = 0; i < games.length; i++) {
		...
		if (now < new Date(game.roundStart) || now > new Date(game.roundEnd)) {
			...
			try {
				...
				const now = new Date();
				const maxTime =
					(game.pubMktStart + game.pubMktMaxTime) * 60 * 60 * 1000; // 78 hours in milliseconds
				...
				for (let i = 0; i < items.length; i++) {
					...
					const content = {
						key: "market.expired.private",
						params: {
							itemName: eraArray[empire.era][itemName.toLowerCase()],
						},
					};
					const pubContent = {
						key: "market.expired.public",
						params: {
			...
			try {
				...
				if (totalTickets > 1) {
					...
					if (!winner || totalTickets < 1 || winningTicket < 1) {
						...
						const content = {
							key: "lottery.noWinner.private",
							params: {
								jackpot: jackpot.toLocaleString(),
							},
						};
						const pubContent = {
							key: "lottery.noWinner.public",
							params: {
								jackpot: jackpot.toLocaleString(),
							},
						};
				...
				try {
					...
					for (let i = 0; i < empires.length; i++) {
						...
						for (const key of Object.keys(empire.achievements)) {
